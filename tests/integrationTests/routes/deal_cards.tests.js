...
describe("Deal Cards Route Tests", function(){
    describe("deal a hand", function(){
        it("should deal two cards to 5 players for Texas Hold 'em", function( done ){
        ...
        it("should deal 5 cards to 4 players for 5 Card Draw", function( done ){
            var cardsInDeck = shuffledDeck;
        ...
        it("should deal 3 cards to the Community (flop) for Texas Hold 'em", function( done ){
            var cardsInDeck = shuffledDeck;
            var gameConfig = gameModel.getBy( "name", "tex");
        ...
        it("should deal 1 card to the Community (turn) for Texas Hold 'em", function( done ){
            var cardsInDeck = shuffledDeck;
            var gameConfig = gameModel.getBy( "name", "tex");
        ...
        it("should deal 1 card to the Community (river) for Texas Hold 'em", function( done ){
            var cardsInDeck = shuffledDeck;
            var gameConfig = gameModel.getBy( "name", "tex");
    ...
    describe("errors", function(){
        it("should error if no cards send in payload", function( done ){
            var gameConfig = gameModel.getBy( "name", "tex");
            agent.get( config.appUrl + '/deal_cards/1/5')
                .query( { "gameConfig": JSON.stringify( gameConfig ) } )
                .then( res =>{
                ...
                .catch( err =>{
                    ...
                    assert.equal( err.response.body.message, "No cards in deck." );
                    done( );
                });
        });
        it("should error if no game congif send in payload", function( done ){
            var cardsInDeck = shuffledDeck;
            agent.get( config.appUrl + '/deal_cards/1/5')
                .query( { "cardsInDeck": JSON.stringify( cardsInDeck )} )
                .then( res =>{
                ...
                .catch( err =>{
                    assert.equal( err.status, 500 );
                    assert.equal( err.response.body.message, "No game configuration." );
                    done( );
                });
        });
        it("should error if the round isn't indicated", function( done ){
            var cardsInDeck = shuffledDeck;
            var gameConfig = gameModel.getBy( "name", "tex");
            agent.get( config.appUrl + '/deal_cards')
                .query( { "cardsInDeck": JSON.stringify( cardsInDeck ) } )
                .query( { "gameConfig": JSON.stringify( gameConfig ) } )
                .then( res =>{
                    assert.equal( res.status, 500 );
                ...
                .catch( err =>{
                    assert.equal( err.status, 500 );
                    assert.equal( err.response.body.message, "I don't know the round to deal." );
                    done( );
                });
        });
        it("should error if an invalid round is requested", function( done ){
            var cardsInDeck = shuffledDeck;
            var gameConfig = gameModel.getBy( "name", "tex");
            agent.get( config.appUrl + '/deal_cards/8')
                .query( { "cardsInDeck": JSON.stringify( cardsInDeck ) } )
                .query( { "gameConfig": JSON.stringify( gameConfig ) } )
                .then( res =>{
                    assert.equal( res.status, 500 );
                ...
                .catch( err =>{
                    assert.equal( err.status, 500 );
                    assert.equal( err.response.body.message, "There is no round 8 for Texas Hold 'em" );
                    done( );
                });
        });
    });
